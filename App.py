...
def pagina_articulo(titulo_articulo):
    ...
    if articulo_encontrado:
        ...
        return render_template("Plantilla_Articulos.html", temas = articulo_encontrado["tema"],
            articulos_procesados = articulos_procesados, nombre = session.get("Usuario", ""))
    else:
        abort(404)

@app.route("/Buscador")
def buscador():
    return render_template("Buscador.html", articulos = articulos, 
        nombre = session.get("Usuario", ""))
